...
class TradingEnvironment(gym.Env):
    """
    Trading environment for PPO agent to learn trading with focus on transaction cost reduction.
    ...
    def __init__(
        ...
        position_penalty: float = 0.005,  # Penalty for staying in a position too long
        max_position_duration: int = 10,  # Maximum duration to stay in a position
    ):
        """
        ...
        Args:
            ...
            position_penalty: Penalty for staying in a position too long
            max_position_duration: Maximum number of steps to stay in a position
        """
        super().__init__()
        ...
        obs_shape = (len(self.feature_columns) + 4,)  # +4 for position, balance, trade count, position duration
    ...
    def _reset_sim_vars(self):
        ...
        self.position_duration = 0  # How long we've been in the current position
    ...
    def _get_observation(self):
        ...
        # Normalize position duration for the observation
        position_duration_normalized = np.array([self.position_duration / self.max_position_duration])
    ...
    def step(self, action):
        ...
        # Apply position duration penalty if staying in position too long
        if old_position != 0:
            self.position_duration += 1
            if self.position_duration > self.max_position_duration:
                position_penalty = self.position_penalty * (self.position_duration - self.max_position_duration)
        ...
        # Calculate reward: returns after costs + directional reward - trade penalty - position penalty
        reward = net_return * self.reward_scaling + directional_reward - trade_penalty - position_penalty
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            step_info = {
                'step': self.current_step,
                'price': self.data.iloc[self.current_step][self.price_columns['close']],
                'position': self.position,
                'balance': self.balance,
    ...
    def get_trade_history(self):
        ...
        return pd.DataFrame(self.trade_history) if self.trade_history else None 